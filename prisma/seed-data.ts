...
async function main() {
  ...
  if (!user) {
    user = await prisma.user.create({
      data: {
        ...
        name: 'Administrador',
        passwordHash: '$2b$10$YourHashedPasswordHere', // Hash do bcrypt
        role: 'ADMIN',
        status: 'ACTIVE',
        tenantId: tenant.id,
      },
  ...
  for (let i = 0; i < 45; i++) {
    ...
    const customer = await prisma.customer.create({
      data: {
        ...
        segment: ['LEAD', 'PROSPECT', 'REGULAR', 'VIP'][Math.floor(Math.random() * 4)] as any,
        tenantId: tenant.id,
      },
    });
  ...
  // Criar 90 produtos (smartphones, acessórios e peças)
  ...
  for (const accessory of accessories) {
    ...
    const product = await prisma.product.create({
      data: {
        ...
        brand: 'Genérico',
        model: 'Universal',
        category: ['ACCESSORY_CASE', 'ACCESSORY_CHARGER', 'ACCESSORY_SCREEN_PROTECTOR'][Math.floor(Math.random() * 3)] as any,
        costPrice,
        salePrice,
        profitMargin: 70,
